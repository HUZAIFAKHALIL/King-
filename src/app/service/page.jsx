...
export const dynamic = 'force-dynamic'; // Add dynamic directive
...
// Create a component for the service logic
        ...
        .then((data) => {
          if (data && data.length > 0) {
            ...
            const approvedServices = data.filter(
              (service) => service.isApproved === true && service.isRejected === false
            );
      ...
      case "restaurant":
        return (
          <div className="space-y-3">
            ...
            {service.specificService && service.specificService.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {service.specificService.map((restaurant, idx) => (
                  <div key={idx} className="w-full p-3 bg-gray-50 rounded-lg">
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-medium capitalize">
                    ...
                    {restaurant.features && (
                      <div className="flex flex-wrap gap-2">
                        {restaurant.features.split(",").map((feature, idx) => (
                          <span
                            key={idx}
                            className="flex items-center gap-1 px-2 py-1 bg-gray-100 rounded-full text-xs text-gray-600"
                          >
                            {getFeatureIcon(feature, "restaurant")}
                            {feature.trim()}
                          </span>
                        ))}
                      </div>
...
// Wrap the ServiceContent component in Suspense
export default function ServiceProducts() {
  return (
